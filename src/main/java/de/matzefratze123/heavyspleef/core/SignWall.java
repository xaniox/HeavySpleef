...
public class SignWall extends RegionCuboid implements DatabaseSerializeable {
	...
	public SignWall(int id, Location corner1, Location corner2) {
		...
		positiveAttached = usesX ? getAttachedBlock(getFirstPoint().getBlock()).getLocation().getBlockX() - getFirstPoint().getBlockX() < 0 ? false : true : getAttachedBlock(getFirstPoint().getBlock()).getLocation().getBlockZ() - getFirstPoint().getBlockZ() < 0 ? false : true;
