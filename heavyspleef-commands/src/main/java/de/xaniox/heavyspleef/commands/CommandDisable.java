...
public class CommandDisable {
	...
	public void onDisableCommand(CommandContext context, HeavySpleef heavySpleef) throws CommandException {
		...
		CommandValidate.isTrue(game.getGameState() != GameState.DISABLED, i18n.getVarString(Messages.Command.GAME_ALREADY_DISABLED)
				.setVariable("game", gameName)
				.toString());
		game.disable();
		
		sender.sendMessage(i18n.getVarString(Messages.Command.GAME_DISABLED)
				.setVariable("game", gameName)
				.toString());
	}
	
	@TabComplete("disable")
	public void onDisableTabComplete(CommandContext context, List<String> list, HeavySpleef heavySpleef) {
		GameManager manager = heavySpleef.getGameManager();
